...
void workerThreadStart(WorkerArgs *const args) {
  ...
  double startTime;
  if (PRINT_TIME) startTime = CycleTimer::currentSeconds();
  if (IMPROVE) {
    for (int i = args->threadId; i < static_cast<int>(args->height);
         i += args->numThreads) {
    ...
    int start =
        ...
        (args->height % args->numThreads < static_cast<unsigned>(args->threadId)
    ...
    int num =
        ...
        (args->height % args->numThreads > static_cast<unsigned>(args->threadId)
             ...
             : 0);
    mandelbrotSerial(args->x0, args->y0, args->x1, args->y1, args->width,
                     args->height, start, num, args->maxIterations,
                     args->output);
  }
  if (PRINT_TIME)
    std::cout << args->threadId << " : "
              << CycleTimer::currentSeconds() - startTime << std::endl;
}
