...
#pragma omp parallel for shared(lastcol, firstcol, r) private(j) reduction(+ : rho)
...
#pragma omp parallel for shared(lastrow, firstrow, rowstr, a, p, colidx, q) private( \
    j, k, sum)
        ...
        d = 0.0;
#pragma omp parallel for shared(lastcol, firstcol, p, q) private(j) reduction(+ : d)
        for (j = 0; j < lastcol - firstcol + 1; j++) {
          d = d + p[j] * q[j];
        }
        alpha = rho / d;
        rho0 = rho;
        rho = 0.0;
#pragma omp parallel for shared(lastcol, firstcol, p, q, z, r, alpha) private(j)
        for (j = 0; j < lastcol - firstcol + 1; j++) {
          z[j] = z[j] + alpha * p[j];
          r[j] = r[j] - alpha * q[j];
        }
#pragma omp parallel for shared(lastcol, firstcol, r) private(j) reduction(+ : rho)
        ...
        beta = rho / rho0;
#pragma omp parallel for shared(lastcol, firstcol, r, p, beta) private(j)
        for (j = 0; j < lastcol - firstcol + 1; j++) {
          p[j] = r[j] + beta * p[j];
        }
      }
      ...
      sum = 0.0;
#pragma omp parallel for shared(lastrow, firstrow, rowstr, a, z, colidx, \
                                r) private(j, k, d)
      for (j = 0; j < lastrow - firstrow + 1; j++) {
        d = 0.0;
        for (k = rowstr[j]; k < rowstr[j + 1]; k++) {
